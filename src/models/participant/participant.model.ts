...
export async function dbGetParticipantByPhone(phone: string) {
	const participant = Participant.findOne({ phone });
	if (!participant) {
...
export async function dbUpdateParticipant(update: Partial<ParticipantType>, filter: UpdateParticipant) {
